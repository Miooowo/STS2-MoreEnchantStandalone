...
> **给 AI / Cursor**：机器侧优先加载 [`.cursor/rules/more-enchant-workflow.mdc`](.cursor/rules/more-enchant-workflow.mdc)（`alwaysApply`），以下为完整说明与协作备忘；若与 `.mdc` 摘要不一致，**以本文件及仓库内 `MoreEnchantStandalone.csproj` 实际配置为准**，并请同步更新 `.mdc`。

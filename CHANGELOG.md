...
- **究极打击紫字显示**：修复究极打击附魔在卡面紫字中显示翻倍（`+28`）的问题，显示与实际结算统一为 `+14`。
- **究极防御卡面格挡**：究极防御按 `Nimble` 同链路加算格挡，卡面顶部数字与实际结算均可直接体现 `+11`。
- **究极防御附魔即时显示**：附魔落到卡牌时显式触发一次数值刷新，拿到附魔当下即可看到卡面格挡数字变化。
- **究极防御预览高亮**：事件预览中将格挡变化写入 `PreviewValue`（不改基础值），卡面数字可显示绿色差值高亮。

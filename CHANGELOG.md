...
### 新增
- 战斗附魔：**坚毅**（打出消耗一张手牌）、**破灭**（打出顶牌并消耗）、**快速**（抽 1）、**放血**（失去 3 生命，获得 2 能量）、**武装**（升级一张手牌）、**恐怖**（稀有，999 层易伤）、**战栗**（2 层易伤）、**中和**（1 层虚弱）、**背包**（抽 1 再丢 1）、**星光**（获得 2 辉星）、**护卫**（召唤 5）、**多打**（罕见，力量攻击段数 +{Amount}，可叠）、**碎屑**（力量攻击伤害 ×4/3，打出将同名未附魔复制品置入手牌）。
- 技术：`Hook.ModifyAttackHitCount` 后缀补丁以支持**多打**；`ValuePropCombatUtil` 复制游戏内力量攻击判定（`IsPoweredAttack` 在 sts2 中为 internal）。
...
### 修复
- **华丽** ×3 曾对所有带 `Move` 的结算生效；**Omnislice（全向斩）** 对次要目标使用 `Unpowered|Move` 并传入已乘过附魔的主目标伤害，导致溅射再×3。现与锐锋等一致，仅对 **Powered** 攻击/格挡（`Move` 且无 `Unpowered`）应用乘区。
- **羞耻 / 疑虑**诅咒附魔：`IHoverTip` 引用与 `HoverTipFactory.FromPower` 返回值类型修正，避免编译失败。

...
- 修复联机下 **恶魔护盾（Demon Shield）** 目标选择可能跨端不一致导致格挡发放错误与 checksum 分叉的问题：统一联机目标选择流程并恢复可手动选择指定队友。
- 腐朽附魔的中文描述问题。
...
- 修复联机下 **恶魔护盾（Demon Shield）** 会在非持有者客户端也弹出选人箭头、并在取消/误选时触发分叉的问题：联机中仅允许牌拥有者端执行目标选择 UI。
